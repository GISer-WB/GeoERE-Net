...
class GCU(Module):
	...
	def GraphProject_optim(self, X):
		...
		Q = torch.div(Q,norm[:,:,None])
		Q = torch.unsqueeze(Q,1)#Q = torch.reshape(self.batch, self.re , self.ht*self.wdth , self.no_of_vert)# #torch.unsqueeze(Q,1)
		...
		norm = z**2 
		#print("norm1 size",norm.shape)
		norm = torch.sum(norm, dim=1) 
		#print("norm2 size",norm.shape) #16,8
		norm = torch.unsqueeze(norm,1)
		#print("norm3 size",norm.shape) #16,8
		#print("Z \n",Z.shape)
		Z = torch.div(z,norm) #1,256,8
		#print("Z \n",Z.shape)
	...
	def GraphReproject(self, Z_o,Q):
		...
		#print ('X_new.shape',X_new.shape) #1,4096,256
		return torch.reshape(X_new, (self.batch, self.outfeatures, self.ht, self.wdth)) #1,256,64,64

	def forward(self, X):
		X = torch.reshape(X,(self.batch, self.ht, self.wdth, self.d)).float() 
		Q, Z, Adj = self.GraphProject_optim(X)
		Q = torch.squeeze(Q,1) 
		out = torch.matmul(torch.transpose(Z,1,2), self.weight) #1,8,256
		out = torch.matmul(Adj, out) #1,8,256
		Z_o = F.relu(out) #1,8,256
		#print("Prinitng Z_o",Z_o.shape) 
